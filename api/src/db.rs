...
pub async fn connect() -> Result<(), Error> {
    ...
    info!("Connecting to SurrealDB at {}", host);
    DB.connect::<ws::Wss>(host).await?;
...
pub async fn create_event(id: String, event: NewEvent) -> Result<Event, Error> {
...
pub async fn update_event(id: String, event: NewEvent) -> Result<Event, Error> {
